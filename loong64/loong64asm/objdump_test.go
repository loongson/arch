...
func allowedMismatchObjdump(text string, inst *Inst, dec ExtInst) bool {
	...
	var objdump_text string = strings.Join(decsp, " ")
	var decode_text string = strings.Replace(text, ",", "", -1)
	//movgr2fcsr $r3(fcsr),$r3(rj), movfcsr2gr $r3(rd), $r3(fcsr)
	if decode_text == strings.Replace(objdump_text, "$", "", -1) || decsp[0] == "movfcsr2gr" || decsp[0] == "movgr2fcsr" {
		return true
	}
	return false
}
